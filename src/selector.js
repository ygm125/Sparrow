define(['lang','data'], function(){
    ...
    var Selector = {
        ...
        adapter : function( selector, context, nextSelector ){
            ...
            return Selector.finder[ type ]( selector, context, nextSelector );
        },
    ...
    Selector.finder = {
        ...
        CLASS : function( selector, context ){        
            ...
            context = Selector.finder.TAG( tagName, context, true );            
            len = context.length;
        ...
        ATTR : function( selector, context, isFiltered ){
            var    elems = [],
                ...
                getAttribute = Selector.getAttribute,
                attr = matches[1],
            ...
            context = isFiltered ? context : Selector.adapter( selector, context );
            len = context.length;
            sMatches = Selector.adapter( selector );
            filterBase = Selector.filter[ sMatches[0] ];
            name = sMatches[1];
            tagName = sMatches[2];
        ...
        RELATIVE : function( selector, context, nextSelector ){
            var    matches = Selector.adapter( nextSelector ),
                type = matches[0],
                filter = Selector.filter[type] || type,
                name = matches[1] || nextSelector;
                
            return Selector.filter.relatives[ selector ]( filter, name, matches[2], context );
        },
        ...
        PSEUDO : function( selector, context, isFiltered ){
            ...
            if( ~pseudoType.indexOf('-') ){
                ...
                switch( type ){
                    case 'nth' :
                        ...
                        parent = Selector.unique( parent );
                        
                        // 索引过滤器
                        filterFn = Selector.indexFilter;
                    ...
                    case 'only' : 
                        ...
                        context = Selector.unique( context, true );
                        len = context.length;
            ...
            // 处理非索引伪类选择器
                ...
                if( filterName ){
                    sMatches = Selector.adapter( filterName );
                }
    ...
    Selector.filter = {
        ...
        pseudos : {
            ...
            animated : function( elem ){
                return S.data( elem, 'anim', 'animQueue' ) !== undefined;
            }
        ...
        relatives : {
            ...
            '>' : function( filter, name, tagName, context ){                
                ...
                return isType ? Selector.finder[filter](name, elems, true) : elems;
            },    
            ...
            '+' : function( filter, name, tagName, context ){
                ...
                for( ; i < len; i++ ){
                    nextElem = Selector.next( context[i] );
    ...
    S.extend( S, {
        ...
        unique : function( nodelist ){
            return Selector.unique( nodelist );
        },
        ...
        filter : function( source, selector ){
            ...
            source = S.makeArray( source );
            len = source.length;
                
            if( S.isString(selector) ){
                matches = Selector.adapter( selector );
                filter = Selector.filter[ matches[0] ];
                name = matches[1];
                tagName = matches[2];
                if( !filter ){
                    type = matches[0];
                ...
                if( type ){
                    target = Selector.finder[ type ]( selector, source, true );
                }
                else{
                    for( i = 0; i < len; i++ ){
                        elem = source[i];
        ...
        query : function( selector, context ){
            ...
            for( i = 0; i < len; i++ ){
                ...
                for( var j = 0, clen = selectors.length; j < clen; j++ ){
                    ...
                    elem = Selector.adapter( result, lastElem, nextMatch );    
            ...
            // 逗号选择器要删除重复的DOM元素
            return len > 1 ? S.unique( elems ) : elems;
        }
